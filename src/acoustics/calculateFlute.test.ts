...
import { embouchureCorrection, openEndCorrection, openHoleCorrection } from './corrections.ts'
...
const standardInput = {
  ...
  toneHoles: [200, 400, 500, 700, 900, 1100].map((cents) => ({ cents, diameterMm: 7 })),
  temperatureC: 20,
  plugOffsetMm: 20.93,
  plugThicknessMm: 12,
  headMarginMm: 8,
...
test('correction terms match documented equations', () => {
  ...
  assert.ok(Math.abs(embouchureCorrection(20.93, 10, 2.87) - 29.000) < 0.01)
})
...
test('lip plate chimney changes acoustic dimensions', () => {
  ...
  const plated = calculateFlute({ ...standardInput, embouchureChimneyMm: 4.37 })
